...
export default function Sidebar() {
    ...
    return (
        <div className={`sticky left-0 top-0 bottom-0 h-screen p-3 pt-8 relative duration-500 border-r-2 border-r-gray-100`} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
            <div className={`inline-flex h-14`}>
                ...
                <h1 className={`text-gray-800 text-2xl mt-1 ${!open && "hidden"}`}>E-Diary</h1>
            </div>
            <hr className='color-red'></hr>
            <ul className='mt-5'>
                {menus.map((menu, index) => {
                    return (
                                <NavLink to={menu.navi} key={index} className={({ isActive }) => isActive ? "bg-gray-100 text-indigo-500 rounded-lg border-2 border-indigo-500 text-gray-500 flex items-center hover:rounded-lg hover:bg-gray-100 p-3 gap-x-4 mt-2" : "border-2 border-white text-gray-500 flex items-center hover:rounded-lg hover:bg-gray-100 p-3 gap-x-4 mt-2"}>
